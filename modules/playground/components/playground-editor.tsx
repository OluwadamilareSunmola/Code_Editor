...
import { InlineCodeAction } from "./inline-code-action"
...
export const PlaygroundEditor = ({
  ...
  const handleEditorDidMount = (editor: any, monaco: Monaco) => {
    ...
    editor.addCommand(monaco.KeyCode.Escape, () => {
      ...
      // Also close code action widget
      setCodeActionState({ position: null, selectedCode: "", selection: null })
    })

    // Add command for AI code action (Cmd/Ctrl + K)
    editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyK, () => {
      const selection = editor.getSelection()
      const selectedText = editor.getModel()?.getValueInRange(selection)

      if (selectedText && selectedText.trim()) {
        // Get the position to show the widget
        ...
        if (coords) {
          ...
          if (containerRect) {
            setCodeActionState({
              position: {
                top: containerRect.top + coords.top + coords.height + 10,
  ...
  // Handle applying code action - replaces selected code with AI suggestion
  const handleApplyCodeAction = useCallback(
    (modifiedCode: string) => {
      if (editorRef.current && monacoRef.current && codeActionState.selection) {
        const editor = editorRef.current
        const monaco = monacoRef.current
        const selection = codeActionState.selection

        // This will REPLACE the selected range with the new code
        // The selection range acts as the area to delete/replace
        const success = editor.executeEdits("code-action-replace", [
          {
            range: selection, // The exact range of selected code to replace
            text: modifiedCode, // The new code to insert in its place
            forceMoveMarkers: true,
          },
        ])
  ...
  return (
    <div className="h-full relative">
      ...
      {/* Inline Code Action Widget */}
      <InlineCodeAction
        position={codeActionState.position}
        selectedCode={codeActionState.selectedCode}
        onClose={() => setCodeActionState({ position: null, selectedCode: "", selection: null })}
        onApply={handleApplyCodeAction}
        fileContent={content}
        language={activeFile ? getEditorLanguage(activeFile.fileExtension || "") : "plaintext"}
      />
    </div>
